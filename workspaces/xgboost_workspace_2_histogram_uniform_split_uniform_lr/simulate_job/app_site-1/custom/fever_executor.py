...
dir_path = '/home/dnde/Documents/PhD/2023/EV_Energy_FL/'

class FedXGBHistogramFEVERExecutor(FedXGBHistogramExecutor):
    ...
    def _read_FEVER_with_pandas(self, data_path, start: int, end: int):
        data_size = end - start
        self.logger.info(f"Reading data from {data_path} with start={start} and end={end}")
    ...
    def load_data(self):
        ...
        # training
        X_train, y_train, total_train_data_num = self._read_FEVER_with_pandas(
            data_path=data_path, start=site_index["start"], end=site_index["end"]
        )
        dmat_train = xgb.DMatrix(X_train, label=y_train)

        # validation
        X_valid, y_valid, total_valid_data_num = self._read_FEVER_with_pandas(
